...
(function ($, window, document, undefined) {
    $(function () {
        ...
        $('#input-mask-image-file').on('change', function () {
            ...
            if (file) {
                ...
                setTimeout(function () {
                    ...
                    form_data.append("ali2woo_nonce", a2wl_wc_pe_script.nonce_action)
                    $.ajax({
                        url: a2wl_wc_pe_script.ajaxurl,
                        dataType: 'json',
                        cache: false,
                        contentType: false,
                        processData: false,
        ...
        function loadImage(image_url) {
            ...
            if(image_url){
                if(a2wl_isExternal(image_url)){
                    jQuery.post(a2wl_wc_pe_script.ajaxurl, {
                        action: 'a2wl_edit_image_url',
                        url: image_url,
                        ali2woo_nonce: a2wl_wc_pe_script.nonce_action,
        ...
        $(".product_images .image .a2wl-photo-edit").click(function () {
            ...
            let data = {
                action: 'a2wl_get_image_by_id',
                attachment_id: $(this).parents('.image').attr('data-attachment_id'),
                ali2woo_nonce: a2wl_wc_pe_script.nonce_action,
            };
            jQuery.post(a2wl_wc_pe_script.ajaxurl, data).done(function (response) {
        ...
        $("#a2wl-edit-image .save-image").click(function () {
            ...
            var save = function () {
                ...
                fd.append("ali2woo_nonce", a2wl_wc_pe_script.nonce_action);
                ...
                jQuery.ajax({
                    ...
                    if (json.state !== 'ok') {
                        ...
                        if ($('#_thumbnail_id[value="'+current_attachment_id+'"]').length > 0) {
                            ...
                            $('.image[data-attachment_id="'+current_attachment_id+'"] img').attr(
                                'src',
                                json.croped_attachment_url?json.croped_attachment_url:json.attachment_url
                            );
                            $('.image[data-attachment_id="'+current_attachment_id+'"]').attr(
                                'data-attachment_id',
                                json.attachment_id
                            );

                            let saveGalleryAttachmentIds = $('input#product_image_gallery').val();

                            saveGalleryAttachmentIds = saveGalleryAttachmentIds ?
                                saveGalleryAttachmentIds + "," + json.attachment_id :
                                json.attachment_id;

                            $('input#product_image_gallery').val(saveGalleryAttachmentIds);
                        } else {
                            //import list
                            $('img[data-id="'+json.image_id+'"]').attr(
                                'src',
                                json.croped_attachment_url ? json.croped_attachment_url : json.attachment_url
                            );
                            $('img[data-id="'+json.image_id+'"]').attr(
